...
const isEnumRequired = require('../utils/is-enum-required');
...
module.exports = function({
  ...
  const componentClassCode = generate(
    componentName,
    componentClass,
    componentName
  );
  const childClassesCode = Object.entries(childClasses)
    .map(([name, propType]) => generate(name, propType, componentName))
    .join('\n\n');

  const classes = `${imports}\n${componentClassCode}\n\n${childClassesCode}`;

  const indentedClasses = addIndentation(classes);
          ...
          assignmentNode.right.elements.reduce(
            (accum, enumProperty, index, array) => {
              ...
              if (index === 0 && isNullable) {
                if (!isNumber) {
                  accum += `${_indent}[EnumMember(Value = "")]\n`;
                }
