...
// Returns a definition object if the prop has a type with a child (argument), like shape, arrayOf, oneOf
const createSingleDefinition = (propName, propType, parent) => {
  const { argument, type } = propType;

  if (type === 'shape') {
    const properties = { ...propType, argument: flattenTypes(argument) };
    const definition = { name: propName, parent, properties };
    const childDefinitions = createShallowDefinitions(argument, {
      name: propName,
      parent
    });
...
module.exports = function(propTypes, className) {
  ...
  const componentClass = {
    ...
    properties: {
      type: 'shape',
      argument: flattenTypes(propTypes),
