...
module.exports = function({ baseClass, indent, namespace, sourceCode }) {
  ...
  const propTypes = parseTree(syntaxTree);

  const newTransform = require('./transforms/transform-prop-types2');

  const transformedPropTypes = newTransform({ propTypes, propTypesMeta });
  ...
  const classes = createDefinitions(transformedPropTypes, componentName);
  ...
  const classesToString = require('./stringify/classes-to-string');

  const string = classesToString({
    baseClass,
    classes,
    componentName,
    indent,
  ...
  return { componentName, code: string };

  // Stringify!
  const code = ASTToCsharp({
    baseClass,
    componentName,
    indent,
    namespace,
