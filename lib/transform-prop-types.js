...
const transformSingleProp = (typeNode, meta = {}) => {
  ...
  const { argument } = typeNode;

  if (meta.type === 'exclude') return;
  if (shouldStrip(typeNode) || shouldStrip(argument)) return;

  if (isShape(typeNode) || isShape(argument)) {
    const func = isShape(argument) ? transformSingleProp : transformPropTypes;
    return { ...typeNode, type, argument: func(argument, meta.argument) };
  }
...
function transformPropTypes(propTypes, propTypesMeta = {}) {
  ...
  return Object.entries(propTypes).reduce((accum, [propName, typeNode]) => {
    try {
