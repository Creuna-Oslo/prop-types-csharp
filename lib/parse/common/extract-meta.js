...
const makeNode = type => ({ type });
const makeNodeWithArg = type => children => ({ type, children });
...
function getMetaNode(node) {
  return match(
    ...
    [t.isIdentifier, Pipe(get('name'), makeNode)],
    [t.isObjectExpression, Pipe(getMetaProperties, makeNodeWithArg('shape'))],
    [t.isArrayExpression, Pipe(getFirstElement, makeNodeWithArg('arrayOf'))],
