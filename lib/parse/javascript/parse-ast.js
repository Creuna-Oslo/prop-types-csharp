...
const parseProperty = meta => ({ key, value }) => ({
  [key.name]: parseType(value, get(`argument.${key.name}`)(meta))
});
...
module.exports = function(syntaxTree, propTypesMeta = {}) {
  const propTypesLiteral = match(
    [t.isFile, get('program.body[0].expression.right')],
    [t.isExpressionStatement, get('expression.right')]
  )(syntaxTree);

  if (isComponentReference(propTypesLiteral)) {
    return propTypesLiteral.object.name;
  }

  return get('properties', [])(propTypesLiteral).reduce((accum, node) => {
