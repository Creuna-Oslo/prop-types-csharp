...
const createAssignment = (name, value) =>
  t.expressionStatement(t.assignmentExpression('=', t.identifier(name), value));
...
module.exports = function({ componentName, syntaxTree }) {
  ...
  if (typeLiteral) {
    ...
    return createAssignment(componentName, propTypesValue);
  }
  ...
  traverse(syntaxTree, {
    ClassProperty(path) {
      ...
      if (key.isIdentifier({ name: 'propTypes' })) {
        propTypesAST = createAssignment(componentName, path.node.value);
        path.stop();
      }
