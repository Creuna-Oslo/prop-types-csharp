...
const validateArrayExpression = (node, propertyName) => {
  ...
  if (!t.isIdentifier(element) && !t.isObjectExpression(element)) {
    throw new Error(`Unsupported value in meta type for '${propertyName}'`);
  }
};
  ...
  matchNode(
    node,
    {
      ...
      ArrayExpression: node => {
        validateArrayExpression(node, propertyName);
        return {
          type: 'arrayOf',
          argument: getMetaNode(node.elements[0])
        };
      }
    },
    () => {
