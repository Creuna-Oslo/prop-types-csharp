  ...
  traverse(syntaxTree, {
    CallExpression(path) {
      ...
      const missingLiteralError = argumentName =>
        new Error(
          `Couldn't resolve 'oneOf' value for prop '${propName}'. Make sure that '${argumentName}' is defined in the above file and that it's not imported.`
        );
      ...
      if (t.isCallExpression(oneOfArgument) && isObjectMethod(oneOfArgument)) {
