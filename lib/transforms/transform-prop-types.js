...
const transformSingleProp = (propName, propType, meta = {}) => {
  ...
  if (propType.argument && propType.argument.type === 'shape') {
    return {
      ...propType,
      argument: transformSingleProp(propName, propType.argument, meta.argument)
    };
  }
...
function transformPropTypes(propTypes, propTypesMeta = {}) {
  ...
  return Object.entries(propTypes).reduce((accum, [propName, propType]) => {
    ...
    return { ...accum, [propName]: transformedProp };
  }, {});
}
